...
/**
  ...
  keycloakAdminClient(settings)
    .then((client) => {
      client.users.getSocialAccounts(realmName, options)
        .then((userList) => {
        console.log(userList) // [{...},{...}, ...]
      })
    })
 */
function getSocialAccounts (client) {
  return function getSocialAccounts (realm, options) {
    return new Promise((resolve, reject) => {
      options = options || {};
      let req = {
        auth: {
      ...
      if (options.userId) {
        req.url = `${client.baseUrl}/admin/realms/${realm}/users/${options.userId}/federated-identity`;
      } else {
        return reject("userId required for identifying user");
      }
...
/**
  A function to add a social account to a user.
  @param {string} realmName - The name of the realm(not the realmID) - ex: master
  @param {object} [options] - The options object
  @param {string} [options.userId] - use this options to get a user by an id. If this value is populated, it overrides the querystring param options
  @param {object} [federatedIdentityRepresentation] An object representing the social identity
  @param {string} [federatedIdentityRepresentation.identityProvider] The provider of the social identity
  @param {string} [federatedIdentityRepresentation.userId] The unique userId in the social provider
  @param {string} [federatedIdentityRepresentation.userName] The unique userName (or email) in the social provider.
  @returns {Promise} A promise that will resolve with an Array of user objects or just the 1 user object if userId is used
  @example
  keycloakAdminClient(settings)
    .then((client) => {
 ...
 */
function addSocial (client) {
  return function addSocial (realm, user, federatedIdentityRepresentation) {
    return new Promise((resolve, reject) => {
      console.log(`${client.baseUrl}/admin/realms/${realm}/users/${user.id}/federated-identity/${federatedIdentityRepresentation.identityProvider}`);
      let req = {
        url: `${client.baseUrl}/admin/realms/${realm}/users/${user.id}/federated-identity/${federatedIdentityRepresentation.identityProvider}`,
        auth: {
          bearer: privates.get(client).accessToken
        },
